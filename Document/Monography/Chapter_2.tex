...
The three signatures of convection in stars are described by David Gray in his research as the principal characteristics to identify the convective motion in the spectrum (see~\cite{Gray_2009,Gray_Oostra_2018,Gray_Pugh_2012} and references therein).
...
For the project purposes only the thermal, rotation and convection broadening effects were studied.
...
\begin{equation} 
    f(\Delta \lambda) = L_D\exp\parens{\frac{-mc^2}{2\lambda^2k_{B}T}\Delta \lambda}
    \label{eq:Doppler broadening}
\end{equation}
Where $L_D$ refers to the line depth, $k_B$ to the Boltzmann constant; and $\Delta \lambda/\lambda$ to the redshift.
The Equation~\eqref{eq:core curvature} describes the line core curvature $C_c$ can be quantified by the second derivative of the line's intensity profile with respect to wavelength, evaluated at observed wavelength.
\begin{equation}
    C_c =  \lambda_{obs}^2 \parens{\frac{d^2 p(\lambda_{obs})}{d \lambda_{obs}^2}}
    \label{eq:core curvature}
\end{equation}
A theorical slope can be derived from Equation~\eqref{eq:Doppler broadening} using the definition of line core curvature as shows the Relation~\eqref{eq:theory slope only doppler}.
\begin{equation}
...
Equation~\eqref{eq:theory slope only doppler} represents the line core curvature slope for lines whic are only affected by the rotation effect.
Directly, assuming the three target effects as independent\footnote{This is not all true, for the convective process is known that greater temperatures leads to more negative convective velocities. For simplicity, the approximation was taken on the project.} the sum of each effect variance is the variance of the radial velocity.
This leads to the Equation~\eqref{eq:Theory slope} of a theoritical slope including the three broadening effects.
\begin{equation}
...
With these values, an approximation of convection variance velocity is expected to be calculated for each atlas.
...
As mentioned before, line profile asymmetries are the principal instrument to measure convection process from the solar atmosphere since most of stellar observations are made with lower-resolution spectrographs and often lower signal-to-noise ratios. 
Gray and Oostra propose a model for the line profile asymmetry to be observable under these conditions, known as the \textit{flux deficit}~\cite{Gray_Oostra_2018}. 
This can be interpreted as flux imbalances on the redshifted side of the line profile, caused by a net loss of radiative energy. 
To quantify the imbalance, the authors proposed calculating the flux deficit by taking the difference between the observed, altered profile, and the third signature characteristic curve.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Flux deficit illustration.png}
    \caption{The first illustration describe the target profile, obtaining the flux deficit curve for the wavelength $\SI{6253}{\angstrom}$ from the WSA spectrum. The second illustration shows the standard curve from the third signature plot and the line profile bisector, features which are fundamental to obtain the flux deficit. Image taken from~\cite{Gray_Oostra_2018}.}\label{fig:flux deficit}%
\end{figure}

As illustrates Figure~\ref{fig:flux deficit}, treating the altered line profile as an approximate representation of the third signature of convection, it can be observed the flux difference interpreted as radiation~\cite{Gray_Oostra_2018}.
This curve gives a sight for the distribution of velocities along the flux in individual lines.
...
Notice in Figure~\ref{fig:granulation pattern ellwarth} the wavelength-dependence on convective blueshift, which have been extensively documented for several datasets.
...
The measured negative redshift resulting from convective motions is known as \textit{convective blueshifts}, which is measured using the relativistic Doppler effect (see Equation~\eqref{eq:relative velocity}). 
\begin{equation}
    v_r \approx c \parens{\frac{ \lambda_{obs}- \lambda_{rep}}{\lambda_{rep}}} -633\text{m/s}
    \label{eq:relative velocity}
\end{equation}
Where the value of $633$ m/s is related to the correction of gravitational redshift for the Sun.
...
\section{Anomaly chromodependence}
In previous research many authors pointed out the phenomenon that we call chromodependence or wavelength dependence on the signatures, emphasizing on the third granulation plot. Because this definition can be ambiguous a clarification is presented below.

In the photosphere model the temperature is higher in the deepest layers than the surface. For the formation of an absorption line, the temperature gives the energy necessary to be prepared in the lowest energy level for transition to the atom. This is defined as the excitation potential for the lowest level of energy $(\chi)$. While lines with higher values of $(\chi)$ are formed on the deepest layers, lines with smaller values of $(\chi)$ are formed on the surface. 

However, we recall the fact mentioned about the opaqueness of the photosphere, the lines formed on the surface show more intensity than the deepest ones.   
This results in absorption lines in the violet range, produced when the atom absorbs a high value of energy, formed on the deepest layers implying a lower excitation potential and a small temperature. This is a chromodependence, but is not the phenomenon which is not expected on the granulation patterns.
In this scheme, all the lines have a convective blueshift whenever the formation layers, in lines for the violet range the blueshift is less than infrared ones. Thereby, the correlation between line depth and velocity illustrated the respective granulation curve. However, this should represent an unique granulation curve, which does not happen at all.
What this project tried to characterize is the anomaly chromodependence, the observational fact of granulation patterns showing individual behaviors with wavelength dependence.
