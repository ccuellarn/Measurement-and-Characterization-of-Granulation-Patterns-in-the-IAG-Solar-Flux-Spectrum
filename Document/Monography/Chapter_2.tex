...
The three signatures of convection in stars are described by David Gray in his research (see~\cite{Gray_2009,Gray_Oostra_2018,Gray_Pugh_2012} and references therein) as the principal characteristics to identify the convective motion in the spectrum.

\subsection{First signature of convection: Line broadening} 
To talk about line broadening is necessary to establish the process that represent a spectral line. 
An individual atom making a transition between energy levels emits one photon with a well-defined frequency.
This can be represented as a graph of radiance or intensity per unit wavelength against wavelength, what is called \textit{line profile} (Landeu).
...
The dynamical and atomic processes on the photosphere causes the broadening effects mention below.
For this project, only the thermal, rotation and convection broadening effects were studied.

Atoms in a gas have random motions with temperature dependence, which mean speed is obtained by the relation between kinetic and thermal energy for gasses. 
The fraction of atoms in a speed interval between $v$ and $v + \Delta v$ is then given by the Gaussian distribution in Equation~\eqref{eq:Gaussian distribution}.
\begin{equation} 
    f(v_{\text{rad}}) = \exp\parens{\frac{-mv_{\text{rad}}^2}{2kT}}
    \label{eq:Gaussian distribution}
\end{equation}
Comparing with a Gaussian distribution centered on the origin, we can relate the width $\sigma^2$ to the variance of the radial velocity (see Equation~\eqref{eq:velocity variance}).
\begin{equation} 
    	\langle v_{\text{rad}}^2 \rangle = \sigma^2 = \frac{kT}{m} 
...
\begin{equation} 
    f(\Delta \lambda) = P\exp\parens{\frac{-mc^2}{2\lambda^2kT}\Delta \lambda}
    \label{eq:Doppler broadening}
\end{equation}
Where $P$ refers to the line depth and $\Delta \lambda/\lambda$ to the redshift.
...
\end{equation}
Assuming that the three efects mentioned of thermal, rotation and convection are independents\footnote{This is not all true, but for simplicity on the project we take the approximation.} the variance of the velocity is the sum of variances of each one.
For hydrogen atoms at the solar effective temperature T = 5800 K, vth ≈ 7 km/s, and the rotation velocity deriving by sun model spherical and solid of (), we have te next relation for a theoritical slope between curvature and line depth.
As in the IAG spatially spectrum the roattion is nule, we expected a better value of convection as we can see clearly this phenomena.

On the other hand, the absorption lines can be classified by decreasing intensity line profile as strongest and weakest lines~\cite{Gray_Pugh_2012}.
In general, stronger or deepest absorption lines exhibit a more pronounced line core curvature than weaker or shallow lines.
...
This phenomenon modify the line core curvature of determining lines profiles, made the line core curvatures to be displaced.
