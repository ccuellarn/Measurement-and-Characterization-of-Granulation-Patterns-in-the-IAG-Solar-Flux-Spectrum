...
This was initially unexpected, as rotational doppler broadening is negligible at the disk center, therefore rotation is not a cause for wavelength shifts.

Furthermore, the shift on every lineal fit at fixed velocity expresses the granulation relation.
The relation between granulation curves on the disk-integrated flux spectrum is not uniform, describes a fluctuates displacement around $-200$ m/s.
This indicates that wavelenght displacements on this part for the spectrum increases for values less to the relative velocitiy form teh sun, and decreases for greater values.
In the disk-center spectrum the behavior across wavelength shifts is uniform, describing a cuadratic increasing.
For this last atlas the limb darkening phenomenon is neligible, what leads to the conclusion of this effect produces a nonuniform shifts.
On the other hand, the relations described above are presented on the H and J band respectively for the near infrared range for the IAG solar flux atlas.

\subsection{Line depth-dependence on excitation potential for the lowest energy level}
The coefficients curve depends strongly on wavelength and weakly on excitation potential.
...
Equation~\eqref{eq:standard line NIR} and Equation~\eqref{eq:standard line VIS} describes the standard granulation curves for all the wavelength range in the IAG solar flux atlas.
Therefore, gives a description about the anomaly chromodependence.
Both of them dependends on the wavelength and the velocity shift.

\subsection{The line core bisector slope}
Has a line bisector slope shift along line depth determined by a slope, so its linear
This concords  with the literarute cause this shit are due to the fe I line displacement.
