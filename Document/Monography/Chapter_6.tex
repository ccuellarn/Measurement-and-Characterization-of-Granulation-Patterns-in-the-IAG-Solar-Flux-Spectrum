...
The function \lstinline|np.polyfit()|, during the process of calculating the fourth-order polynomial fit, presents an over estimation on the coefficients due to the large difference of magnitude order between axes.
...
\chapter{The third derivative relation}\label{ap:third derivative}

The third derivative relation or called the line core bisector slope was deducted by Professor Benjamin and showed below.

Define the slope for the minimum point as Equation~\eqref{eq:slope}, where the points $b,c,h$ are illustrated in Figure~\eqref{fig: CBS slope}.
\begin{equation}
...
\begin{figure}[H]
    ...
    \caption{Illustration for the points $a$, $b$, $c$ and the height whose defines the line core bisector slope (see Equation~\eqref{eq:slope}).}\label{fig: CBS slope}
\end{figure}

Note that in a symmetrical line the bisector is vertical and the slope, according to this definition, is zero.
Now, the curve may be expanded around the point $a$ by Taylor series.
...
\label{eq:CBS}
\end{equation}
In the code it was multiplied by the relation ($c/\lambda$) to see the behavior across line depth.
...
\begin{figure}[H]
    ...
    \caption{Tools bar for the visualizer, contains a classification system of labels, the option to save images, and add to a Dataframe for dropping.}\label{fig:tools bar}
\end{figure}
...
If is necessary to change the label, use the button "$\times$ Clear" first and then give another label to the line. 
...
To make a list of lines which are gonna to be deleted or filtered use the button "+ Add", which includes the line core information with the label given.
...
The option for "Save JPG" of the graph is available, saving the image in format JPG and showing the confirmation message with the index and wavelength of the line (see Figure~\ref{fig:save image}).
...
\begin{figure}[H]
    ...
    \caption{Navigation bar for the visualizer, help to locate a specific line or explorating the differents graphs using a slider, writing box, and buttons for moves.}\label{fig:Navigations bar}
\end{figure}
...
We discard any lines with distances between minimums and Fe I lines exceeding $\SI{0.1}{\angstrom}$.
...
The function \lstinline|width_minimums| selecting the bins of $\SI{50}{\miliangstrom}$ and $\SI{0.2}{\angstrom}$ around the minimal point, respectively for the line core and the line profile.
...
\begin{figure}[H]
    ...
    \caption{Examples for discarted lines with the first filter of the visualizer. The line cores and line bisectors of the two plots shows one or more of the conditions presented.}\label{fig:1st filter examples blend}
\end{figure}
...
The next cell saves the filtered lines in an excel file.
...
This function calculate the observed wavelength based on the fourth-order polynomial fit.
...
\begin{figure}[H]
    ...
    \caption{Examples for discarted lines with the first filter of the visualizer.}\label{fig:2nd filter examples blend}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Examples for accepted lines with the first filter of the visualizer.}\label{fig:2nd filter examples}
\end{figure}
...
After the process, the resulting excel is the definitive blend-free list of Fe I lines.
