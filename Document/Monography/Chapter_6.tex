...
Based on the definition for the standardization, the derivatives of the expression~\eqref{eq:z score} have to follow Equation~\eqref{eq:re scaled derivates}.
\begin{equation}
...
\begin{figure}[H]
    ...
    \caption{Illustration for the points $a$, $b$, $c$ and the height which defines the line Core Bisector Slope CBS (see Equation~\eqref{eq:slope}).}\label{fig: CBS slope}
\end{figure}
...
\end{equation}
The point $a$ is chosen at the minimum, so $f'(a)=0$ and $f(c)-f(a)=h$, given Equation~\eqref{eq:Taylor expansion}.
\begin{equation}
...
\end{equation}
As both variables has identical equations, $x$ and $y$ are two of the equations roots which leads to define the CBS like Equation~\eqref{eq:CBS new}.
\begin{equation}
 CBS =  \lim_{h\to 0} \frac{x+y}{2h}
 \label{eq:CBS new}
\end{equation}
Of the three possible roots of a cubic equation, the two that we need are those that tend to Equation~\eqref{eq:CBS roots}
\begin{equation}
 \lim_{h\to 0} \frac{x}{y} = -1
 \label{eq:CBS roots}
\end{equation}
...
\begin{equation}
 ax^3+bx^2+cx+d=0 \quad \rightarrow \quad a=\frac{f'''(a)}{3} \quad \quad b=f''(a) \quad \quad c=0  \quad \quad d=-2h
\label{eq:3rd fit coeff}
\end{equation}
A standard procedure is to change the variable $x$ to produced a reduced equation as follows.
\begin{equation}
x=t - \frac{b}{3a}= t - \frac{f''(a)}{f'''(a)}
\end{equation}
...
\begin{equation}
 t^3+pt+q=0 \quad \rightarrow \quad p=-3\parens{\frac{f''(a)}{f'''(a)}}^2 \quad q = 2\frac{(f''(a))^3-3h(f'''(a))^2}{(f'''(a))^3}
 \label{eq:3rd fit}
\end{equation}
The Equation~\eqref{eq:3rd fit} have three solutions given Equation~\eqref{eq:solutions}.
\begin{equation}
 t_i = 2\parens{\frac{f''(a)}{f'''(a)}}\cos\parens{\frac{1}{3}\arccos\parens{\frac{3h(f'''(a))^2}{(f''(a))^3} - 1} -\frac{2\pi i}{3}} \quad \quad i=0,1,2
\end{equation}
When $h$ vanishes, there are two equal solutions $\cos(\theta) = 1/2$ and one differing $\cos(\theta)= -1$.
Taking a parameter $\epsilon$, when $h$ vanishes $\epsilon <<1$, we abbreviated the expresion to Equation~\eqref{eq:epsilon}.
\begin{equation}
\epsilon = \frac{h(f''')^2}{(f'')^3} <<1 \quad \quad  t_i = 2\parens{\frac{f''(a)}{f'''(a)}}\cos\parens{\frac{1}{3}\arccos\parens{3\epsilon - 1} -\frac{2\pi i}{3}} 
\label{eq:epsilon}
\end{equation} 
%------------------------------
In the limit $h\to 0$, $\delta=\sqrt{6\epsilon}$ and the trigonometric expression were restricted as Equation~\eqref{eq:delta}.
\begin{equation}
\arccos(3\epsilon -1)=\pi-\delta \quad \quad \delta<<1 \quad \rightarrow \quad 3\epsilon -1 = \cos(\pi -\delta) = -\cos(\delta) \approx \frac{\delta^2}{2}-1
\label{eq:delta}
\end{equation}
Therefore, the solutions for the Taylor expansion are expressed by Equation~\eqref{eq:solutions}.
\begin{equation}
t_{\pm} = 2\frac{f''(a)}{f'''(a)}\parens{\frac{1}{2}\cos\parens{\frac{\sqrt{6\epsilon}}{3}} \mp \frac{\sqrt{3}}{2}\sin\parens{\frac{\sqrt{6\epsilon}}{3}} } 
\label{eq:solutions}
\end{equation}
For the line core bisector slope appears the condition~\eqref{eq:line core condition}.
\begin{equation}
x+y = t_+ + t_- - 2\parens{\frac{f''(a)}{f'''(a)}} = -\frac{2}{3}\parens{\frac{f''(a)}{f'''(a)}}\parens{\frac{h(f'''(a))^2}{(f''(a))^3}} = -2h\frac{f'''(a)}{3(f''(a))^2}
\label{eq:line core condition}
\end{equation}
Whence, finally, the core bisector slope is defined by Equation~\eqref{eq:CBS}
\begin{equation}
CBS =  -\frac{f'''(a)}{3(f''(a))^2}
\label{eq:CBS}
\end{equation}
