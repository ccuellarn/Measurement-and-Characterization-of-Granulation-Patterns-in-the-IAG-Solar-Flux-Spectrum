...
\end{figure}
The value for the variance and the standard deviation in each wavelength give a level of accuracy in the showed results.

Consequently, a plot of emitted wavelength across observed for each window width was realized (see Figure~\ref{fig:variance for window}). 
...
\end{figure}
The optimal window is determined by the minimal window width with the closest value for one, which demonstrates a consistency window width for a line core representing precision.
...
The mentioned methodology implements the Nave list of laboratory-measured Fe I lines~\cite{Nave_1994} which presents laboratory measurements for wavelength and excitation potential for the lowest and highest energy level. 
